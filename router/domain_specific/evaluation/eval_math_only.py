"""
Math-Only Evaluation: Baseline MF vs Domain MF Router
======================================================
Filters test set to ONLY Math queries and compares both routers.

Usage:
    cd router/domain_specific
    python eval_math_only.py
"""
...
def main():
    ...
    print(f"\n{'='*65}")
    print("  MATH-ONLY Evaluation: Baseline MF vs Domain MF Router")
    ...
    math_mask = test_ds.domain_ids.numpy() == 1
    math_count = math_mask.sum()
    print(f"  Total test samples : {len(test_ds):,}")
    print(f"  Math test queries  : {math_count:,}  ({math_count/len(test_ds)*100:.1f}%)")

    # ── 2. Get ALL predictions (domain model) ─────────────────────────────────
    ...
    # ── 3. Get ALL predictions (baseline model) ───────────────────────────────
    ...
    for emb, lbl in DataLoader(test_standard, batch_size=256, shuffle=False):
        all_emb_b.append(emb); all_lbl_b.append(lbl)
    emb_b  = torch.cat(all_emb_b).to(device)

    with torch.no_grad():
        base_probs = baseline(emb_b).cpu().numpy().squeeze()

    # ── 4. Filter to ONLY Math queries ────────────────────────────────────────
    math_domain_probs = domain_probs[math_mask]
    math_base_probs   = base_probs[math_mask]
    math_labels       = lbl_np[math_mask]

    always_strong_quality = math_labels.mean()
    print(f"\n  Always-GPT4 baseline quality (Math only): {always_strong_quality*100:.2f}%")

    # ── 5. Probability distribution on math queries ───────────────────────────
    print(f"\n{'='*65}")
    print("  CONFIDENCE SCORES ON MATH QUERIES")
    print(f"{'='*65}")
    print(f"  {'Metric':<35} {'Baseline':>12} {'Domain MF':>12}")
    print(f"  {'-'*60}")
    print(f"  {'Mean Probability':<35} {math_base_probs.mean():>11.4f}  {math_domain_probs.mean():>11.4f}")
    print(f"  {'Median Probability':<35} {np.median(math_base_probs):>11.4f}  {np.median(math_domain_probs):>11.4f}")
    print(f"  {'Std Dev of Probability':<35} {math_base_probs.std():>11.4f}  {math_domain_probs.std():>11.4f}")
    print(f"  {'% Queries routed to GPT-4 (t=0.5)':<35} {(math_base_probs>=0.5).mean()*100:>10.2f}%  {(math_domain_probs>=0.5).mean()*100:>10.2f}%")

    # ── 6. Accuracy and F1 (at threshold = 0.5) ──────────────────────────────
    base_preds   = (math_base_probs   >= 0.5).astype(int)
    domain_preds = (math_domain_probs >= 0.5).astype(int)

    base_acc   = (base_preds   == math_labels).mean() * 100
    domain_acc = (domain_preds == math_labels).mean() * 100
    base_f1    = f1_score(math_labels, base_preds,   zero_division=0) * 100
    domain_f1  = f1_score(math_labels, domain_preds, zero_division=0) * 100

    print(f"\n{'='*65}")
    print("  ACCURACY & F1 ON MATH QUERIES (threshold=0.5)")
    print(f"{'='*65}")
    ...
    # ── 7. Cost vs Quality sweep — Math Only ─────────────────────────────────
    base_sweep   = threshold_sweep(math_base_probs,   math_labels)
    domain_sweep = threshold_sweep(math_domain_probs, math_labels)

    print(f"\n{'='*65}")
    print("  COST vs QUALITY ON MATH QUERIES ONLY")
    ...
    for drop in [0.0, 0.5, 1.0, 2.0, 3.0, 5.0]:
        b_cost, b_qual, b_t = cost_at_drop(base_sweep,   math_labels, drop)
        d_cost, d_qual, d_t = cost_at_drop(domain_sweep, math_labels, drop)
